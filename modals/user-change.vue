<template>
  <div>
    <!-- <div id="user-view-header" class="h-10 bg-gray-800 hover:bg-gray-700 text-white hover:text-white">
      <div v-if="status == 'adding'" class="pl-4">
        Add new user
      </div>
      <div v-else-if="status == 'changing'" class="pl-4">
        Change user
      </div>
      <div class="hidden sm:block absolute top-0 right-0 pr-4">
        <button type="button" class="rounded-md text-xl text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2" @click="onClose('canceled')">
    ...
    <div>
      <div class="flex justify-center py-4">
        <div class="flex bg-purple-200 rounded-full md:p-4 p-2 border-2 border-purple-300">
          <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
        </div>
      </div>
...
export default {
  ...
  methods: {
    onClose (e) {
      this.$emit('close')
      this.$root.$emit('user-change-modal', e)
    }
  }
}
</script>
